...
class ServiceStack extends Stack {
  constructor(scope: App, id: string, props?: StackProps) {
    ...
    const container = task.addContainer('c', {
      ...
      memoryLimitMiB: 256, cpu: 128, logging: ecs.LogDriver.awsLogs({ streamPrefix: 'tank-adventure' })
    });
    container.addPortMappings({ containerPort: 4005, protocol: Protocol.TCP });
    ...
    const tg = new ApplicationTargetGroup(this, 'Tg', {
      protocol: ApplicationProtocol.HTTP, port: 4005, vpc: (cluster as any).vpc, targetType: TargetType.INSTANCE
    });
    (service.node.defaultChild as any).addPropertyOverride('LoadBalancers', [{ ContainerName: 'c', ContainerPort: 4005, TargetGroupArn: tg.targetGroupArn }]);
    ...
    listener.addAction('route-tank-adventure-4005', {
      action: ListenerAction.forward([tg]),
      conditions: [ListenerCondition.pathPatterns(['/game/*'])],
...
const app = new App();
new ServiceStack(app, `Tank-adventureStack-${process.env.STAGE || 'dev'}`);
