...
class ServiceStack extends Stack {
  constructor(scope: App, id: string, props?: StackProps) {
    ...
    const container = task.addContainer('c', {
      ...
      memoryLimitMiB: 256, cpu: 128, logging: ecs.LogDriver.awsLogs({ streamPrefix: 'lobby' })
    });
    container.addPortMappings({ containerPort: 4002, protocol: Protocol.TCP });
    ...
    const tg = new ApplicationTargetGroup(this, 'Tg', {
      protocol: ApplicationProtocol.HTTP, port: 4002, vpc: (cluster as any).vpc, targetType: TargetType.INSTANCE
    });
    (service.node.defaultChild as any).addPropertyOverride('LoadBalancers', [{ ContainerName: 'c', ContainerPort: 4002, TargetGroupArn: tg.targetGroupArn }]);
    ...
    listener.addAction('route-lobby-4002', {
      action: ListenerAction.forward([tg]),
      conditions: [ListenerCondition.pathPatterns(['/lobby/*'])],
...
const app = new App();
new ServiceStack(app, `LobbyStack-${process.env.STAGE || 'dev'}`);
