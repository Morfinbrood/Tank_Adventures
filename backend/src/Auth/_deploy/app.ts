...
class ServiceStack extends Stack {
  constructor(scope: App, id: string, props?: StackProps) {
    ...
    const container = task.addContainer('c', {
      ...
      memoryLimitMiB: 256, cpu: 128, logging: ecs.LogDriver.awsLogs({ streamPrefix: 'auth' })
    });
    container.addPortMappings({ containerPort: 4001, protocol: Protocol.TCP });
    ...
    const tg = new ApplicationTargetGroup(this, 'Tg', {
      protocol: ApplicationProtocol.HTTP, port: 4001, vpc: (cluster as any).vpc, targetType: TargetType.INSTANCE
    });
    (service.node.defaultChild as any).addPropertyOverride('LoadBalancers', [{ ContainerName: 'c', ContainerPort: 4001, TargetGroupArn: tg.targetGroupArn }]);
    ...
    listener.addAction('route-auth-4001', {
...
const app = new App();
new ServiceStack(app, `AuthStack-${process.env.STAGE || 'dev'}`);
