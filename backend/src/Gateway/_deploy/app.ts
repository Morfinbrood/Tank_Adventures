...
class ServiceStack extends Stack {
  constructor(scope: App, id: string, props?: StackProps) {
    ...
    const container = task.addContainer('c', {
      ...
      memoryLimitMiB: 256, cpu: 128, logging: ecs.LogDriver.awsLogs({ streamPrefix: 'gateway' })
    });
    container.addPortMappings({ containerPort: 4100, protocol: Protocol.TCP });
    ...
    const tg = new ApplicationTargetGroup(this, 'Tg', { protocol: ApplicationProtocol.HTTP, port: 4100, vpc: (cluster as any).vpc, targetType: TargetType.INSTANCE });
    (service.node.defaultChild as any).addPropertyOverride('LoadBalancers', [{ ContainerName: 'c', ContainerPort: 4100, TargetGroupArn: tg.targetGroupArn }]);
    const listener = ApplicationListener.fromApplicationListenerArn(this, 'L', listenerArn);
    listener.addAction('route-gateway-4100', {
...
const app = new App();
new ServiceStack(app, `GatewayStack-${process.env.STAGE || 'dev'}`);
